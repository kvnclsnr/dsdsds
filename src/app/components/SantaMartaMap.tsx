...
export function SantaMartaMap({
  ...
  useEffect(() => {
    ...
    if (ambosCarriles) {
      // En ambos carriles: no respetar sentidos, solo unir puntos siguiendo calles.
      const fetchBothLanesSegments = async () => {
          ...
          if (points[i].disconnected) {
            if (currentChunk.length > 1) {
              const coordsStr = currentChunk
                .map((p) => `${p.lng},${p.lat}`)
                .join(";");
              const url = `https://router.project-osrm.org/route/v1/foot/${coordsStr}?geometries=geojson&overview=full&alternatives=false&steps=false`;
              try {
                const res = await fetch(url);
                const data = await res.json();
                if (data.code === "Ok" && data.routes?.length > 0) {
                  allSegments.push(
                    toLatLng(data.routes[0].geometry.coordinates),
                  );
                } else {
                  allSegments.push(
