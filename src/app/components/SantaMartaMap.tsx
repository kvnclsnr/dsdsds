    ...
    if (ambosCarriles) {
      ...
      const fetchSegments = async () => {
        ...
        for (let i = 0; i < points.length - 1; i++) {
          ...
          try {
            const [resAB, resBA] = await Promise.all([fetch(urlAB), fetch(urlBA)]);
            const [dataAB, dataBA] = await Promise.all([resAB.json(), resBA.json()]);

            const routeAB = (dataAB.code === 'Ok' && dataAB.routes?.length > 0) ? dataAB.routes[0] : null;
            const routeBA = (dataBA.code === 'Ok' && dataBA.routes?.length > 0) ? dataBA.routes[0] : null;

            let coords: [number, number][] = [[pA.lat, pA.lng], [pB.lat, pB.lng]];
            const maxAllowedDist = Math.max(straightDist * 1.35, straightDist + 35);

            if (routeAB && routeBA) {
              const chooseAB = routeAB.distance <= routeBA.distance;
              const best = chooseAB ? routeAB : routeBA;

              if (best.distance <= maxAllowedDist) {
                coords = best.geometry.coordinates.map((c: number[]) => [c[1], c[0]]);
                if (!chooseAB) coords.reverse();
              }
            } else if (routeAB && routeAB.distance <= maxAllowedDist) {
              coords = routeAB.geometry.coordinates.map((c: number[]) => [c[1], c[0]]);
            } else if (routeBA && routeBA.distance <= maxAllowedDist) {
              coords = routeBA.geometry.coordinates.map((c: number[]) => [c[1], c[0]]).reverse();
            }

            if (currentSegmentCoords.length > 0) coords.shift();
